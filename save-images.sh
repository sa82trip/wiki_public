...
for CHANGED_FILE in $CHANGE_LIST; do
    ...
    URI_LIST=`rg "https://user-images\.githubuser.*?\$NUM/.*?(png|jpg|gif)" -o $CHANGED_FILE`
    ...
    for URI in $URI_LIST; do
        ...
        if [ "$?" == "0" ]; then
			...
			sed -n '/image/p' $CHANGED_FILE
            # sed -i '' -E 's/  .*https://.*('"$FILE_NAME"')*//post-img\/'$DIR_NAME'\/\1 ,g' $CHANGED_FILE
			# sed -i 's/https.*('"$FILE_NAME"')/post-img\/'$DIR_NAME'/g' $CHANGED_FILE
			sed -i 's/https.*)/post-img\/'$DIR_NAME'\/'$FILE_NAME')/g' $CHANGED_FILE
            # sed -i 's/http.*.png/photos/g' $CHANGED_FILE
